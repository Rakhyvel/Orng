...
const Evaluated_Const_Args = struct {
    args: std.array_list.Managed(Ast_Id),
...
//     const identifier = ast_.AST.create_identifier(Token.init_simple(stamped_symbol.name), ctx.allocator());
...
//     const domain = Symbol_Tree.extract_domain(template_ast.template.decl.children().*, ctx.allocator());
//     args.* = try args_.default_args(.function, args.*, call_span, domain, &ctx.errors, ctx.allocator());
//     _ = try args_.validate_args_arity(.function, args, domain, false, call_span, &ctx.errors);
...
//     const new_scope = Scope.init(scope, scope.uid_gen, ctx.allocator());

//     // Clone out a new fn decl AST, with a new name
//     // const fn_decl = template_ast.template.decl.clone(ctx.allocator());
...
//         ctx.allocator(),
//     );
//     try fn_symbol.scope.put_symbol(fn_symbol, &ctx.errors);
...
//     try walker_.walk_asts(const_decls, Symbol_Tree.new(new_scope, &ctx.errors, ctx.allocator()));

//     // Decorate identifiers, validate
//     const decorate_context = Decorate.new(new_scope, &ctx.errors, ctx.allocator());
//     const decorate_access_context = Decorate_Access.new(new_scope, &ctx.errors, compiler);
