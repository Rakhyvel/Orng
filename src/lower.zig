...
pub fn lower_AST(
    ...
    switch (ast.*) {
        ...
        .unit_type => {
            ...
            cfg.appendInstruction(ir_.IR.create_ast(lval, ast, ast.getToken().span, allocator));
            return lval;
        },
        .unit_value => {
            const lval = cfg.create_temp_lvalue(primitives_.unit_type, allocator);
            cfg.appendInstruction(ir_.IR.create(.loadUnit, lval, null, null, ast.getToken().span, allocator));
            return lval;
        },
        .int => {
            const temp = cfg.create_temp_lvalue(ast.typeof(allocator), allocator);
            const ir = ir_.IR.createInt(temp, ast.int.data, ast.getToken().span, allocator);
            // temp.def = ir;
        ...
        .char => {
            ...
            const ir = ir_.IR.createInt(temp, codepoint, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        .float => {
            const temp = cfg.create_temp_lvalue(ast.typeof(allocator), allocator);
            const ir = ir_.IR.createFloat(temp, ast.float.data, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        .string => {
            ...
            const temp = cfg.create_temp_lvalue(ast.typeof(allocator), allocator);
            const ir = ir_.IR.createString(temp, cfg.interned_strings.items.len - 1, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        .identifier => {
            ...
            if (symbol.kind == ._fn) {
                ...
                var ir = ir_.IR.create(.loadSymbol, lval, null, null, ast.getToken().span, allocator);
        ...
        ._true => {
            const temp = cfg.create_temp_lvalue(ast.typeof(allocator), allocator);
            const ir = ir_.IR.createInt(temp, 1, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        ._false => {
            const temp = cfg.create_temp_lvalue(ast.typeof(allocator), allocator);
            const ir = ir_.IR.createInt(temp, 0, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        ...
        .not => {
            ...
            const ir = ir_.IR.create(.not, temp, expr, null, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        .negate => {
        ...
        ._try => {
            ...
            const end = ir_.IR.createLabel(cfg, ast.getToken().span, allocator);
            const err = ir_.IR.createLabel(cfg, ast.getToken().span, allocator);
            ...
            cfg.appendInstruction(ir_.IR.createGetTag(condition, expr, ast.getToken().span, allocator)); // Assumes `ok` tag is zero, `err` tag is nonzero
            cfg.appendInstruction(ir_.IR.createBranch(condition, err, ast.getToken().span, allocator));

            // Unwrap the `.ok` value
            const retval_lval = lval_.L_Value.create_unversioned_symbver(cfg.return_symbol, allocator);
            cfg.appendInstruction(ir_.IR.create_simple_copy(retval_lval, expr, ast.getToken().span, allocator));
            cfg.appendInstruction(ir_.IR.createJump(error_label, ast.getToken().span, allocator));
            ...
            cfg.appendInstruction(ir_.IR.createJump(end, ast.getToken().span, allocator));
        ...
        .discard => {
            ...
            const ir = ir_.IR.createDiscard(expr.?, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        ...
        ._or => {
            ...
            // Labels used
            const else_label = ir_.IR.createLabel(cfg, ast.getToken().span, allocator);
            const end_label = ir_.IR.createLabel(cfg, ast.getToken().span, allocator);
            ...
            const branch = ir_.IR.createBranch(lhs, else_label, ast.getToken().span, allocator);
            ...
            const load_true = ir_.IR.createInt(load_true_lval, 1, ast.getToken().span, allocator);
            cfg.appendInstruction(load_true);
            cfg.appendInstruction(ir_.IR.createJump(end_label, ast.getToken().span, allocator));
            ...
            const copy_right = ir_.IR.create_simple_copy(copy_right_lval, rhs.?, ast.getToken().span, allocator);
        ...
        ._and => {
            ...
            const load_false = ir_.IR.createInt(load_false_lval, 0, ast.getToken().span, allocator);
        ...
        .add => {
            ...
            const ir = ir_.IR.create_int_float_kind(.add_int, .add_float, temp, lhs, rhs, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        .sub => {
            ...
            const ir = ir_.IR.create_int_float_kind(.sub_int, .sub_float, temp, lhs, rhs, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        .mult => {
            ...
            const ir = ir_.IR.create_int_float_kind(.mult_int, .mult_float, temp, lhs, rhs, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        .div => {
            ...
            const ir = ir_.IR.create_int_float_kind(.div_int, .div_float, temp, lhs, rhs, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        .mod => {
            ...
            const ir = ir_.IR.create(.mod, temp, lhs, rhs, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        .equal => {
            ...
            // Labels used
            const fail_label = ir_.IR.createLabel(cfg, ast.getToken().span, allocator);
            const end_label = ir_.IR.createLabel(cfg, ast.getToken().span, allocator);

            generate_tuple_equality(cfg, lhs.?, rhs.?, fail_label, allocator);

            cfg.appendInstruction(ir_.IR.createInt(temp, 1, ast.getToken().span, allocator));
            cfg.appendInstruction(ir_.IR.createJump(end_label, ast.getToken().span, allocator));
            cfg.appendInstruction(fail_label);
            cfg.appendInstruction(ir_.IR.createInt(temp, 0, ast.getToken().span, allocator));
            cfg.appendInstruction(end_label);
        ...
        .not_equal => {
            ...
            if (lhs_type.* == .sum) {
                ...
                cfg.appendInstruction(ir_.IR.createGetTag(lhs_tag, lhs.?, ast.getToken().span, allocator));
                cfg.appendInstruction(ir_.IR.createGetTag(rhs_tag, rhs.?, ast.getToken().span, allocator));
                cfg.appendInstruction(ir_.IR.create(.not_equal, temp, lhs_tag, rhs_tag, ast.getToken().span, allocator));
            } else {
                cfg.appendInstruction(ir_.IR.create(.not_equal, temp, lhs, rhs, ast.getToken().span, allocator));
            }
            return temp;
        },
        .greater => {
            ...
            const ir = ir_.IR.create_int_float_kind(.greater_int, .greater_float, temp, lhs, rhs, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        .lesser => {
            ...
            const ir = ir_.IR.create_int_float_kind(.lesser_int, .lesser_float, temp, lhs, rhs, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        .greater_equal => {
            ...
            const ir = ir_.IR.create_int_float_kind(.greater_equal_int, .greater_equal_float, temp, lhs, rhs, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        .lesser_equal => {
            ...
            const ir = ir_.IR.create_int_float_kind(.lesser_equal_int, .lesser_equal_float, temp, lhs, rhs, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        ._catch => {
            ...
            const condition = cfg.create_temp_lvalue(primitives_.word64_type, allocator);
            cfg.appendInstruction(ir_.IR.createGetTag(condition, lhs, ast.getToken().span, allocator)); // Assumes `ok` tag is zero, `err` tag is nonzero
            cfg.appendInstruction(ir_.IR.createBranch(condition, else_label, ast.getToken().span, allocator));
            ...
            cfg.appendInstruction(ir_.IR.create_simple_copy(rhs_lval, rhs, ast.getToken().span, allocator));
            ...
            cfg.appendInstruction(ir_.IR.create_simple_copy(some_lval, val, ast.getToken().span, allocator));
        ...
        ._orelse => {
            ...
            const condition = cfg.create_temp_lvalue(primitives_.word64_type, allocator);
            const load_tag = ir_.IR.createGetTag(condition, lhs, ast.getToken().span, allocator); // Assumes `some` tag is nonzero, `none` tag is zero
            cfg.appendInstruction(load_tag);

            const branch = ir_.IR.createBranch(condition, else_label, ast.getToken().span, allocator);
            cfg.appendInstruction(branch);
        ...
        .call => {
            ...
            var ir = ir_.IR.createCall(temp, lhs, ast.getToken().span, allocator);
            for (ast.call.args.items) |term| {
                ir.data.lval_list.append((try lower_AST(cfg, term, return_label, break_label, continue_label, error_label, errors, allocator)) orelse continue) catch unreachable;
            }
            cfg.appendInstruction(ir_.IR.createStackPush(ast.getToken().span, allocator));
            cfg.appendInstruction(ir);
            cfg.appendInstruction(ir_.IR.createStackPop(ast.getToken().span, allocator));
            return temp;
        },
        .function => {
            const temp = cfg.create_temp_lvalue(ast.typeof(allocator), allocator);
            cfg.appendInstruction(ir_.IR.create_ast(temp, ast, ast.getToken().span, allocator));
            return temp;
        },
        .index => {
            const _type = ast.typeof(allocator);
            ...
            if (ast_expanded_type.* == .product and ast_expanded_type.product.was_slice) {
                ...
                const ir = ir_.IR.createInt(length, ast_expanded_type.product.terms.items.len, ast.getToken().span, allocator);
                cfg.appendInstruction(ir);
            } else {
                unreachable;
            }
        ...
        .select => {
            ...
            if (lhs_expanded_type.* == .sum) {
                ...
                cfg.appendInstruction(ir_.IR.createGetTag(tag.?, ast_lval.?, ast.getToken().span, allocator));
            }
        ...
        .product => {
            if (ast.product.terms.items[0].typeof(allocator).typesMatch(primitives_.type_type)) {
                ...
                const temp = cfg.create_temp_lvalue(ast.typeof(allocator), allocator);
                var ir = ir_.IR.createLoadStruct(temp, ast.getToken().span, allocator);
                for (ast.product.terms.items) |term| {
                    ir.data.lval_list.append((try lower_AST(cfg, term, return_label, break_label, continue_label, error_label, errors, allocator)) orelse return null) catch unreachable;
                }
                cfg.appendInstruction(ir);
                return temp;
            }
        ...
        .addrOf => {
            if ((ast.typeof(allocator)).typesMatch(primitives_.type_type)) {
                ...
                const ir = ir_.IR.create(.addrOf, temp, expr, null, ast.getToken().span, allocator);
                cfg.appendInstruction(ir);
                return temp;
            }
        },
        .subSlice => {
            ...
            { // Dynamically confirm that lower <= upper
                ...
                const ir = ir_.IR.create(.greater_int, compare, lower, upper, ast.getToken().span, allocator);
                cfg.appendInstruction(ir);
                const branch = ir_.IR.createBranch(compare, end_label, ast.getToken().span, allocator);
                cfg.appendInstruction(branch);
                cfg.appendInstruction(ir_.IR.createStackPush(ast.getToken().span, allocator));
                cfg.appendInstruction(ir_.IR.createPanic("subslice lower bound is greater than upper bound", ast.getToken().span, allocator));
                cfg.appendInstruction(end_label);
            }

            const new_size = cfg.create_temp_lvalue(primitives_.int_type, allocator);
            const new_size_ir = ir_.IR.create(.sub_int, new_size, upper, lower, ast.getToken().span, allocator);
            ...
            const new_data_ptr_ir = ir_.IR.create(.add_int, new_data_ptr, data_ptr, lower, ast.getToken().span, allocator);
            cfg.appendInstruction(new_data_ptr_ir);

            const temp = cfg.create_temp_lvalue(ast.typeof(allocator), allocator);
            var load_struct = ir_.IR.createLoadStruct(temp, ast.getToken().span, allocator);
        ...
        .inferredMember => {
            ...
            const ir = ir_.IR.createUnion(temp, init, ast.inferredMember.pos.?, ast.getToken().span, allocator);
            cfg.appendInstruction(ir);
            return temp;
        },
        ...
        ._if => {
            ...
            if (try lower_AST(cfg, ast._if.bodyBlock, return_label, break_label, continue_label, error_label, errors, allocator)) |block_lval| {
                ...
                if (ast._if.elseBlock == null) {
                    ...
                    block_copy = ir_.IR.createUnion(block_copy_lval, block_lval, 1, ast.getToken().span, allocator);
                } else {
                    // regular if-else => copy block
                    block_copy = ir_.IR.create_simple_copy(block_copy_lval, block_lval, ast.getToken().span, allocator);
                }
            ...
            if (ast._if.elseBlock) |elseBlock| {
                if (try lower_AST(cfg, elseBlock, return_label, break_label, continue_label, error_label, errors, allocator)) |else_lval| {
                    ...
                    const else_copy = ir_.IR.create_simple_copy(else_copy_lval, else_lval, ast.getToken().span, allocator);
                ...
                const else_copy_lval = lval_.L_Value.create_unversioned_symbver(symbol, allocator);
                const else_copy = ir_.IR.createUnion(else_copy_lval, null, 0, ast.getToken().span, allocator);
                cfg.appendInstruction(else_copy);
            }
            cfg.appendInstruction(end_label);
            return temp;
        ...
        .match => {
            ...
            const none_label = ir_.IR.createLabel(cfg, ast.getToken().span, allocator);
            ...
            for (ast.match.mappings.items) |_| {
                lhs_label_list.append(ir_.IR.createLabel(cfg, ast.getToken().span, allocator)) catch unreachable;
                rhs_label_list.append(ir_.IR.createLabel(cfg, ast.getToken().span, allocator)) catch unreachable;
            }
            ...
            for (ast.match.mappings.items) |mapping| {
                ...
                cfg.appendInstruction(ir_.IR.createJump(rhs_label_list.items[label_index], mapping.getToken().span, allocator));
                label_index += 1;
            }
            ...
            for (ast.match.mappings.items) |mapping| {
                ...
                if (try lower_AST(cfg, mapping.mapping.rhs.?, return_label, break_label, continue_label, error_label, errors, allocator)) |rhs_lval| {
                    ...
                    if (ast.match.has_else) {
                        rhs_copy = ir_.IR.create_simple_copy(rhs_copy_lval, rhs_lval, ast.getToken().span, allocator);
                    } else {
                        rhs_copy = ir_.IR.createUnion(rhs_copy_lval, rhs_lval, 1, ast.getToken().span, allocator);
                    }
        ...
        ._while => {
            ...
            // Labels used
            const cond_label = ir_.IR.createLabel(cfg, ast.getToken().span, allocator);
            const current_continue_label = ir_.IR.createLabel(cfg, ast.getToken().span, allocator);
            const else_label = ir_.IR.createLabel(cfg, ast.getToken().span, allocator);
            const end_label = ir_.IR.createLabel(cfg, ast.getToken().span, allocator);
            ...
            cfg.appendInstruction(ir_.IR.createJump(cond_label, ast.getToken().span, allocator));

            // Else block
            cfg.appendInstruction(else_label);
            if (ast._while.elseBlock) |elseBlock| {
                if (try lower_AST(cfg, elseBlock, return_label, break_label, continue_label, error_label, errors, allocator)) |else_lval| {
                    const elseCopy = ir_.IR.create_simple_copy(temp, else_lval, ast.getToken().span, allocator);
        ...
        .block => {
            if (ast.block.statements.items.len == 0 and ast.block.final == null) {
                ...
                for (ast.block.scope.?.defers.items) |_| {
                    continue_labels.append(ir_.IR.createLabel(cfg, ast.getToken().span, allocator)) catch unreachable;
                    break_labels.append(ir_.IR.createLabel(cfg, ast.getToken().span, allocator)) catch unreachable;
                    return_labels.append(ir_.IR.createLabel(cfg, ast.getToken().span, allocator)) catch unreachable;
                }
                for (ast.block.scope.?.errdefers.items) |_| {
                    error_labels.append(ir_.IR.createLabel(cfg, ast.getToken().span, allocator)) catch unreachable;
                }
                const end = ir_.IR.createLabel(cfg, ast.getToken().span, allocator);
                ...
                if (ast.block.final) |final| {
                    ...
                    if (current_error_label != null and expanded_temp_type.* == .sum and expanded_temp_type.sum.was_error) {
                        ...
                        const condition = cfg.create_temp_lvalue(primitives_.word64_type, allocator);
                        cfg.appendInstruction(ir_.IR.createGetTag(condition, _temp, ast.getToken().span, allocator)); // Assumes `ok` tag is zero, `err` tag is nonzero
                        const not_condition = cfg.create_temp_lvalue(primitives_.bool_type, allocator);
                        cfg.appendInstruction(ir_.IR.create(.not, not_condition, condition, null, ast.getToken().span, allocator));
                        cfg.appendInstruction(ir_.IR.createBranch(not_condition, current_error_label, ast.getToken().span, allocator));
                    }
                }
                ...
                cfg.appendInstruction(ir_.IR.createJump(break_label, ast.getToken().span, allocator));

                try generateDefers(cfg, &ast.block.scope.?.defers, &return_labels, errors, allocator);
                cfg.appendInstruction(ir_.IR.createJump(return_label, ast.getToken().span, allocator));
        ...
        .fnDecl => {
            ...
            var ir = ir_.IR.create(.loadSymbol, temp, null, null, ast.getToken().span, allocator);
        ...
        ._continue => {
            cfg.appendInstruction(ir_.IR.createJump(continue_label, ast.getToken().span, allocator));
            return null;
        },
        ._unreachable => {
            cfg.appendInstruction(ir_.IR.createStackPush(ast.getToken().span, allocator));
            cfg.appendInstruction(ir_.IR.createPanic("reached unreachable code", ast.getToken().span, allocator));
            return null;
        },
        ._break => {
            cfg.appendInstruction(ir_.IR.createJump(break_label, ast.getToken().span, allocator));
        ...
        ._return => {
            if (ast._return.expr) |expr| {
                ...
                const retval_lval = lval_.L_Value.create_unversioned_symbver(cfg.return_symbol, allocator);
                const retval_copy = ir_.IR.create_simple_copy(retval_lval, retval, ast.getToken().span, allocator);
                ...
                if (expanded_expr_type.* == .sum and expanded_expr_type.sum.was_error) {
                    // Returning error sum, runtime check if error, branch to error path
                    const condition = cfg.create_temp_lvalue(primitives_.word64_type, allocator);
                    cfg.appendInstruction(ir_.IR.createGetTag(condition, retval, ast.getToken().span, allocator)); // Assumes `ok` tag is zero, `err` tag is nonzero
                    const not_condition = cfg.create_temp_lvalue(primitives_.bool_type, allocator);
                    cfg.appendInstruction(ir_.IR.create(.not, not_condition, condition, null, ast.getToken().span, allocator));
                    cfg.appendInstruction(ir_.IR.createBranch(not_condition, error_label, ast.getToken().span, allocator));
                }
                cfg.appendInstruction(ir_.IR.createJump(return_label, ast.getToken().span, allocator));
            } else {
...
fn generate_tuple_equality(
    ...
    if (lhs_type.* == .product) {
        ...
        const lhs_tag = cfg.create_temp_lvalue(primitives_.word64_type, allocator);
        const rhs_tag = cfg.create_temp_lvalue(primitives_.word64_type, allocator);
        cfg.appendInstruction(ir_.IR.createGetTag(lhs_tag, new_lhs, lhs.extract_symbver().symbol.span, allocator));
        cfg.appendInstruction(ir_.IR.createGetTag(rhs_tag, new_rhs, lhs.extract_symbver().symbol.span, allocator));
        cfg.appendInstruction(ir_.IR.create(.equal, temp, lhs_tag, rhs_tag, lhs.extract_symbver().symbol.span, allocator));
        cfg.appendInstruction(ir_.IR.createBranch(temp, fail_label, lhs.extract_symbver().symbol.span, allocator));
    } else {
        cfg.appendInstruction(ir_.IR.create(.equal, temp, new_lhs, rhs, lhs.extract_symbver().symbol.span, allocator));
        cfg.appendInstruction(ir_.IR.createBranch(temp, fail_label, lhs.extract_symbver().symbol.span, allocator));
    }
}
...
) FlattenASTError!?*lval_.L_Value // If assign is to a single symbver, returns the symbver
{
    if (lhs.* == .product) {
        ...
        var ir = ir_.IR.create(.copy, null, rhs, null, lhs.getToken().span, allocator);
        ir.dest = lval;
        ir.safe = true;
        cfg.appendInstruction(ir);
        return null;
...
fn generate_pattern(
    ...
    if (pattern.* == .symbol) {
        if (!std.mem.eql(u8, pattern.symbol.name, "_")) {
            ...
            const ir = ir_.IR.create_simple_copy(symbver, def, pattern.getToken().span, allocator);
            cfg.appendInstruction(ir);
        }
...
fn generate_match_pattern_check(
    ...
    switch (pattern.?.*) {
        ...
        => {
            ...
            const condition_ir = ir_.IR.create(.equal, condition, new_expr, value.?, pattern.?.getToken().span, allocator);
            cfg.appendInstruction(condition_ir);
            const branch = ir_.IR.createBranch(condition, next_pattern, pattern.?.getToken().span, allocator);
            cfg.appendInstruction(branch);
        },
        .symbol => {
        ...
        .select => {
            ...
            const sel_ir = ir_.IR.createInt(sel, pattern.?.select.pos.?, pattern.?.getToken().span, allocator);
            cfg.appendInstruction(sel_ir);

            // Get tag of expr
            const tag = cfg.create_temp_lvalue(primitives_.word64_type, allocator);
            const tag_ir = ir_.IR.createGetTag(tag, expr, pattern.?.getToken().span, allocator);
            ...
            const neql_ir = ir_.IR.create(.equal, neql, tag, sel, pattern.?.getToken().span, allocator);
            cfg.appendInstruction(neql_ir);
            const branch = ir_.IR.createBranch(neql, next_pattern, pattern.?.getToken().span, allocator);
            cfg.appendInstruction(branch);
        },
        .inferredMember => {
            // Get tag of pattern
            const sel = cfg.create_temp_lvalue(primitives_.word64_type, allocator);
            const sel_ir = ir_.IR.createInt(sel, pattern.?.inferredMember.pos.?, pattern.?.getToken().span, allocator);
            cfg.appendInstruction(sel_ir);
