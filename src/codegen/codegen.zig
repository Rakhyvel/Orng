...
pub fn output_modules(compiler: *Compiler_Context) !void {
    ...
    for (compiler.packages.keys()) |package_name| {
        ...
        try output(module, build_path, &package.local_modules, compiler.allocator());
    }
}

/// Takes in a statically correct symbol tree, writes it out to a file
/// TODO: Codegen Context that has this method instead
fn output(module: *Module, build_path: []const u8, local_modules: *std.ArrayList(*Module), allocator: std.mem.Allocator) !void {
    if (module.visited) {
        return;
    }
    module.visited = true;
    ...
    var source_emitter = Source_Emitter.init(module, output_c_file.writer());
    ...
    for (module.local_imported_modules.keys()) |child| {
        if (std.mem.eql(u8, child.package_name, module.package_name)) {
            try output(child, build_path, local_modules, allocator);
        }
    }
}
