...
pub fn package_find(ctx: *Compiler_Context, interpreter: *Interpreter_Context, current_module_path: []const u8, package_src: Ast_Id) Error!Package_Find_Result {
    // Construct the path to the package's `build.orng` file
    const package_absolute_path = switch (package_src.pos().?) {
        0 => try package_find_relative(current_module_path, package_src.enum_value.init.?, ctx.allocator()),
        1 => try package_find_git(ctx, package_src.enum_value.init.?),
        else => unreachable,
    };

    const package_build_paths = [_][]const u8{ package_absolute_path, "build.orng" };
    const package_build_path = std.fs.path.join(ctx.allocator(), &package_build_paths) catch unreachable;

    const path_buffer = ctx.allocator().alloc(u8, std.fs.max_path_bytes) catch unreachable;
    ...
    const build_cfg = try ctx.compile_build_file(package_build_absolute_path);
    ...
    const retval_place = lval_.L_Value.create_raw_address_lval(adrs, ctx.allocator());

    // Jump to the `build()` fn
    try interpreter.call(build_cfg.symbol, retval_place, std.array_list.Managed(*lval_.L_Value).init(ctx.allocator()));
    try interpreter.run();

    return .{ .package_adrs = adrs, .package_dirname = package_absolute_path };
}

fn package_find_relative(current_module_path: []const u8, package_src: Ast_Id, allocator: std.mem.Allocator) Error![]const u8 {
...
fn package_find_git(ctx: *Compiler_Context, package_src: Ast_Id) Error![]const u8 {
    const git_source_type = ctx.get_core_type("Git_Source");
    const url = package_src.get_field(git_source_type, "url");
    const subdir = package_src.get_field(git_source_type, "subdir");

    try repo_.git_clone(url.string.data, ctx.allocator());

    const repo_dir = repo_.get_packages_dir(ctx.allocator());
    var dirs = std.array_list.Managed([]const u8).init(ctx.allocator());
    defer dirs.deinit();
    ...
    if (subdir.string.data.len > 0) {
        ...
        var subdir_string = String.init_with_contents(ctx.allocator(), subdir.string.data) catch unreachable;
    ...
    return std.fs.path.join(ctx.allocator(), dirs.items) catch unreachable;
}
