/* Code generated using the Orng compiler https://ornglang.org */
#include <math.h>
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>

#include "debug.inc"

/* Forward struct, union, and function declarations */
struct struct2;
struct struct4;

/* Struct, union, and function definitions */
typedef int64_t(*function0)(void);

struct struct2 {
    void* _0;
    void* _1;
};

typedef int64_t(*function1)(void*, void*);

struct struct4 {
    int64_t* _0;
    int64_t* _1;
};

typedef int64_t(*function3)(int64_t*, int64_t*);

/* Trait vtable type definitions */
struct vtable_My_Trait {
    int64_t(*id)(void);
};

/* Function forward definitions */
int64_t _1707_main(void);
int64_t _1700_cmp(void* _1700_$self_ptr, int64_t* _1700_other);
int64_t _1698_id(void);

/* Trait vtable implementations */
struct vtable_My_Trait _1697_$vtable = {
    .id = _1698_id,
};


/* Function definitions */
int64_t _1707_main(void){
    int64_t _1708_x;
    int64_t _1708_y;
    int64_t* _1707_t6;
    int64_t* _1707_t7;
    function3 _1707_t8;
    int64_t _1707_t5;
    int64_t _1707_t9;
    uint8_t _1707_t11;
    int64_t _1707_t12;
    int64_t _1707_t13;
    int64_t _1707_$retval;
    _1708_x = 5;
    _1708_y = 6;
    _1707_t6 = &_1708_x;
    _1707_t7 = &_1708_y;
    _1707_t8 = (function3) _1700_cmp;
    $lines[$line_idx++] = "tests/integration/traits/virtual-non-virtual-mix.orng:25:11:\n    if x.>cmp(&y) == 1 {\n         ^";
    _1707_t5 = _1707_t8(_1707_t6, _1707_t7);
    $line_idx--;
    _1707_t9 = 1;
    _1707_t11 = _1707_t5==_1707_t9;
    if (_1707_t11) {
        goto BB1935;
    } else {
        goto BB1939;
    }
BB1935:
    _1707_t12 = 301;
    $lines[$line_idx++] = "tests/integration/traits/virtual-non-virtual-mix.orng:26:18:\n        301 + x.>id()\n                ^";
    _1707_t13 = _1697_$vtable.id();
    $line_idx--;
    _1707_$retval = $add_int64_t(_1707_t12, _1707_t13, "tests/integration/traits/virtual-non-virtual-mix.orng:26:14:\n        301 + x.>id()\n            ^");
    return _1707_$retval;
BB1939:
    $lines[$line_idx++] = "tests/integration/traits/virtual-non-virtual-mix.orng:28:20:\n        unreachable\n                  ^";
    $panic("reached unreachable code\n");
}

int64_t _1700_cmp(void* _1700_$self_ptr, int64_t* _1700_other){
    int64_t _1701_self;
    uint8_t _1700_t2;
    int64_t _1700_t1;
    int64_t _1700_t5;
    int64_t _1700_$retval;
    uint8_t _1700_t7;
    _1701_self = *(int64_t*)_1700_$self_ptr;
    _1700_t2 = _1701_self>*_1700_other;
    if (_1700_t2) {
        goto BB1917;
    } else {
        goto BB1921;
    }
BB1917:
    _1700_t1 = -1;
    goto BB1920;
BB1921:
    _1700_t7 = _1701_self==*_1700_other;
    if (_1700_t7) {
        goto BB1924;
    } else {
        goto BB1927;
    }
BB1920:
    _1700_$retval = _1700_t1;
    return _1700_$retval;
BB1924:
    _1700_t5 = 0;
    goto BB1926;
BB1927:
    _1700_t5 = 1;
    goto BB1926;
BB1926:
    _1700_t1 = _1700_t5;
    goto BB1920;
}

int64_t _1698_id(void){
    int64_t _1698_$retval;
    _1698_$retval = 4;
    return _1698_$retval;
}


int main(void) {
  printf("%ld",_1707_main());
  return 0;
}
