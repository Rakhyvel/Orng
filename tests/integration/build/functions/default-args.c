/* Code generated using the Orng compiler https://ornglang.org */
#include <math.h>
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>

#include "debug.inc"

/* Typedefs */
typedef struct {
    int64_t _0;
    int64_t _1;
} struct1;

typedef int64_t(*function0)(int64_t, int64_t);

/* Function forward definitions */
int64_t _894_main(void);
int64_t _896_add(int64_t _896_x,int64_t _896_y);

/* Function definitions */
int64_t _894_main(void) {
    function0 _894_t0;
    function0 _894_t2;
    int64_t _894_t4;
    int64_t _894_t5;
    int64_t _894_t3;
    function0 _894_t6;
    int64_t _894_t8;
    int64_t _894_t9;
    int64_t _894_t7;
    int64_t _894_t1;
    int64_t _894_$retval;
    _894_t0 = _896_add;
    _894_t2 = _896_add;
    _894_t4 = 47;
    _894_t5 = 1;
    $lines[$line_idx++] = "tests/integration/functions/default-args.orng:2:27:\nfn main() -> Int {add(add(47), add())}\n                         ^";
    _894_t3 = _894_t2(_894_t4, _894_t5);
    $line_idx--;
    _894_t6 = _896_add;
    _894_t8 = 1;
    _894_t9 = 1;
    $lines[$line_idx++] = "tests/integration/functions/default-args.orng:2:36:\nfn main() -> Int {add(add(47), add())}\n                                  ^";
    _894_t7 = _894_t6(_894_t8, _894_t9);
    $line_idx--;
    $lines[$line_idx++] = "tests/integration/functions/default-args.orng:2:23:\nfn main() -> Int {add(add(47), add())}\n                     ^";
    _894_t1 = _894_t0(_894_t3, _894_t7);
    $line_idx--;
    _894_$retval = _894_t1;
    return _894_$retval;
}

int64_t _896_add(int64_t _896_x,int64_t _896_y) {
    int64_t _896_$retval;
    _896_$retval = $add_int64_t(_896_x, _896_y, "tests/integration/functions/default-args.orng:4:43:\nfn add(x: Int = 1, y: Int = 1) -> Int {x + y}\n                                         ^");
    return _896_$retval;
}

int main(void) {
  printf("%ld",_894_main());
  return 0;
}
