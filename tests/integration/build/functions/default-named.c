/* Code generated using the Orng compiler https://ornglang.org */
#include <math.h>
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>

#include "debug.inc"

/* Typedefs */
typedef struct {
    int64_t _0;
    int64_t _1;
} struct1;

typedef int64_t(*function0)(int64_t, int64_t);

/* Function forward definitions */
int64_t _729_main(void);
int64_t _731_add(int64_t _731_x,int64_t _731_y);

/* Function definitions */
int64_t _729_main(void) {
    function0 _729_t0;
    int64_t _729_t2;
    int64_t _729_t3;
    int64_t _729_t1;
    int64_t _729_$retval;
    _729_t0 = _731_add;
    _729_t2 = 4;
    _729_t3 = 50;
    $lines[$line_idx++] = "tests/integration/functions/default-named.orng:3:9:\n    add(.x = 4)\n       ^";
    _729_t1 = _729_t0(_729_t2, _729_t3);
    $line_idx--;
    _729_$retval = _729_t1;
    return _729_$retval;
}

int64_t _731_add(int64_t _731_x,int64_t _731_y) {
    int64_t _731_$retval;
    _731_$retval = $add_int64_t(_731_x, _731_y, "tests/integration/functions/default-named.orng:6:40:\nfn add(x: Int, y: Int = 50) -> Int {x + y}\n                                      ^");
    return _731_$retval;
}

int main(void) {
  printf("%ld",_729_main());
  return 0;
}
